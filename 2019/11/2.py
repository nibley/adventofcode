...
x = 0
y = 0
grid[ (x, y) ] = 1 # 1 is white

directions = [
    # rearranged to make output more legible
    (-1,  0),
    ( 0,  1),
    ( 1,  0),
    ( 0, -1),
...
sorted_x_values = sorted( position[0] for position in grid )
sorted_y_values = sorted( position[1] for position in grid )
for x in range(sorted_x_values[0], sorted_x_values[-1] + 1):
    for y in range(sorted_y_values[0], sorted_y_values[-1] + 1):
