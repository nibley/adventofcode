...
def visualize():
    min_x, *_, max_x = sorted(x for x, _ in grid.keys())
    min_y, *_, max_y = sorted(y for _, y in grid.keys())

    for row_id in range(min_y, max_y + 1):
        for column_id in range(min_x, max_x + 1):
            if row_id == 0 and column_id == 0:
                display = 'S'
            elif row_id == y and column_id == x:
                display = 'R'
            elif not grid[ (column_id, row_id) ]:
                display = '#'
            elif (column_id, row_id) not in visited:
                display = '?'
            else:
                display = ' '
