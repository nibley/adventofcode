...
neighbor_offsets = tuple(
    offset
    for offset in cartesian_product((-1, 0, 1), repeat=4)
    if offset != (0, 0, 0, 0)
)
...
def get_neighbor_count(position):
    x, y, z, w = position
    return sum(
        grid[ (x + x_offset, y + y_offset, z + z_offset, w + w_offset) ]
        for x_offset, y_offset, z_offset, w_offset in neighbor_offsets
    )
...
for t in range(6):
    ...
    min_x, *_, max_x = sorted(x for x, _, _, _ in active)
    min_y, *_, max_y = sorted(y for _, y, _, _ in active)
    min_z, *_, max_z = sorted(z for _, _, z, _ in active)
    min_w, *_, max_w = sorted(w for _, _, _, w in active)
