...
for _ in range(64):
    for knot_length in KNOT_LENGTHS:
        ...
        if wrap_length < 1:
            ...
            split_point = knot_region_wrap.stop
            rope[ : split_point ] = knot_region_reversed[ -1 * split_point : ]
            rope[ position : ] = knot_region_reversed[ : -1 * split_point ]
...
print(
    ''.join(
        '{:02x}'.format(reduce(xor, rope[ 16 * i : 16 * (i + 1) ]))
        for i in range(16)
    )
)
